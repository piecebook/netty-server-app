...
public class LoginHandler implements PBRequestHandler {
    ...
    public Message process(PBSession session, Message msg) {
        ...
        //需要操作数据库
        String result = accountService.login(msg.getSender(), msg.getContent());

        if ("fail".equals(result) || "unfound".equals(result)) {
            //验证失败
            reply.setContent(result);//result包含失败原因，1、用户名不存在 "user not found" 2、密码错误 "fail"
        } else {
            //用户名、密码 验证成功
            logger.info(msg.getSender() + " login on " + session.getSession().remoteAddress());

            //reply.setParam("st", PBCONSTANT.SUCCESS);//成功标志
            reply.setContent(result);//返回用户id
             ...
             */
            PBSession oldsession = sessionManager.get(msg.getSender());
            if (oldsession == null)
                sessionManager.add(msg.getSender(), session);
